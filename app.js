/* Nova — mobile-first neon player. Sound fix: audio uses CORS + WebAudio graph with fallback. */
'use strict';
...
const EQ_PRESETS = { 'Normal':[0,0,0,0,0], 'Pop':[-1,2,4,2,-1], 'Rock':[4,3,-1,3,4], 'Jazz':[3,2,0,2,3], 'Bass 🔊':[7,5,2,0,0], 'Vocal':[-2,1,4,3,1] };
const NEONS = ['#00f0ff','#ff2fd6','#a3ff00','#ffaa00','#b18cff','#ff5c5c'];
...
function rebuild(){ cache = [...DEMO.map(d=>({...d})), ...localTracks];
...
function artStyle(t){ const h=t?.hue??220; return `background:linear-gradient(135deg,hsl(${h} 75% 52%),hsl(${(h+70)%360} 75% 38%))`; }
...
function mediaSession(){ if(!('mediaSession' in navigator)) return; const t=getT(currentId); if(!t) return;
  try{ navigator.mediaSession.metadata=new MediaMetadata({title:t.title,artist:t.artist,album:t.album||'Nova'});
...
function render(){
  ...
  (q?searchFilter(all()).slice(0,8):popular).forEach(t=>{
    ...
    const pb=document.createElement('button'); pb.className='pop-play'; pb.textContent = currentId===t.id&&!audio.paused?'⏸':'▶';
    pb.onclick=(e)=>{ e.stopPropagation(); currentId===t.id?toggle():playTrack(t.id); };
    ...
    c.onclick=()=>playTrack(t.id); pr.appendChild(c);
  });
  ...
  $('#radioToggle').textContent=(!audio.paused&&currentId)?'⏸':'▶';
  const ra=$('#radioArt');
  if(rt?.coverUrl) ra.innerHTML=`<img src="${rt.coverUrl}" style="width:100%;height:100%;object-fit:cover" alt=""/>`;
  else { ra.innerHTML=currentId?'🎵':'📻'; ra.style.cssText=rt?artStyle(rt):''; }
  paint($('#radioNext'), queue.map(getT).filter(Boolean).slice(0,3), store.radio?'Random mix mode 🔀':'Queue is empty');
  // mini + player
  ...
  if(rt){ $('#miniTitle').textContent=rt.title; $('#miniArtist').textContent=rt.artist;
    ...
    $('#plTitle').textContent=rt.title; $('#plArtist').textContent=rt.artist+(rt.album?' · '+rt.album:'');
    ...
    $('#plLyrPreview').textContent=(store.lyrics[currentId]||'').split('\n')[0]||'Tap 📝 below to add lyrics';
  }
  $('#miniPlay').textContent=playing?'⏸':'▶'; $('#plPlay').textContent=playing?'⏸':'▶';
  $('#plShuffle').classList.toggle('off',!store.prefs.shuffle);
  $('#plRepeat').textContent=store.prefs.repeat==='one'?'🔂':'🔁';
  $('#plRepeat').classList.toggle('off',store.prefs.repeat==='off');
  $('#plLike').textContent=currentId&&store.likes.includes(currentId)?'★':'☆';
  $('#plSleepBtn').style.borderColor=sleepId?'var(--neon)':'';
  document.documentElement.style.setProperty('--neon',store.prefs.neon||'#00f0ff');
  document.querySelector('meta[name=theme-color]').content='#070b1a';
  const h=new Date().getHours();
  $('#greetTitle').textContent=(h<12?'Good morning ☀️':h<17?'Good afternoon 👋':'Good evening 🌙');
}
function plRow(pl){
  const d=document.createElement('div'); d.className='track';
  d.innerHTML=`<div class="t-art" style="background:linear-gradient(135deg,var(--neon),var(--neon2))">${esc((pl.name||'P')[0].toUpperCase())}</div><div class="t-meta"><b>${esc(pl.name)}</b><span>${pl.trackIds.length} songs</span></div><button class="t-menu">›</button>`;
...
function songMenu(id){ const t=getT(id); if(!t) return; const liked=store.likes.includes(id);
  ...
  <button class="opt" id="mRen">✏️ Rename</button>
...
function lyrEditor(id){ const t=getT(id);
  sheet(`<h2>📝 ${esc(t.title)}</h2><textarea id="lyT" placeholder="Paste lyrics...">${esc(store.lyrics[id]||'')}</textarea><button class="opt" id="lyS">💾 Save lyrics</button>`);
  $('#lyS').onclick=()=>{ store.lyrics[id]=$('#lyT').value; save(); closeSheet(); render(); toast('Lyrics saved 📝'); }; }
function eqSheet(){
  sheet(`<h2>🎚️ Equalizer</h2><label class="switch"><input type="checkbox" id="eqE" ${store.eq.enabled?'checked':''}/> Enable EQ</label>
...
function setSleep(min){ if(sleepId){ clearTimeout(sleepId); sleepId=null; }
  if(min>0){ sleepId=setTimeout(()=>{ const f=setInterval(()=>{ if(audio.volume>0.08) audio.volume-=0.08; else { clearInterval(f); audio.pause(); audio.volume=(store.volumes.vol||90)/100; } },200); sleepId=null; render(); toast('Sleep timer ended 😴'); },min*60000); toast('Music stops in '+min+' min 😴'); }
...
function queueSheet(){ sheet(`<h2>☰ Up next (${queue.length})</h2><div class="v-list" id="qL"></div><button class="opt" id="qC">🧹 Clear queue</button><button class="opt" id="qX">✕ Close</button>`);
  const ql=$('#qL'); ql.innerHTML=queue.length?'':'<p class="muted">Empty — use ⋮ on any song</p>';
...
function settingsSheet(){
  sheet(`<h2>⚙️ Settings</h2><h3>Neon color</h3><div class="dots">${NEONS.map(c=>`<div class="dot ${store.prefs.neon===c?'active':''}" data-c="${c}" style="background:${c};color:${c}"></div>`).join('')}</div>
  <h3>Playback</h3><div class="eq-row"><label>Speed</label><input type="range" id="sSp" min="0.5" max="2" step="0.05" value="${store.prefs.speed}"/><span>${(+store.prefs.speed).toFixed(2)}×</span></div>
  ...
  <p class="muted">Nova v2 · neon mobile edition · PWA + APK ready</p><button class="opt" id="sX">✕ Close</button>`);
  $('#sX').onclick=closeSheet;
  $$('#sheetBox [data-c]').forEach(d=>d.onclick=()=>{ store.prefs.neon=d.dataset.c; save(); settingsSheet(); render(); });
  ...
  $('#sEx').onclick=()=>{ const b=new Blob([JSON.stringify({store,at:new Date().toISOString()},null,2)],{type:'application/json'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download='nova-backup.json'; a.click(); toast('Exported ⬇'); };
...
function bind(){
  ...
  $('#plVol').addEventListener('input',e=>{ store.volumes.vol=+e.target.value; audio.volume=store.volumes.vol/100; save(); });
  $('#plVol').value=store.volumes.vol||90;
  // radio
